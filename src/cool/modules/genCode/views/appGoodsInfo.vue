<template>
	<cl-crud :ref="setRefs('crud')" @load="onLoad">
		...
		<el-row type="flex" align="middle">
			...
			<cl-query field="status" :list="DictRef.AppGoodsInfoStatusDict" />
			<cl-filter label="类型:">
				<el-select size="mini" v-model="condition.types" @change="typesChange">
					<el-option value="" label="全部" />
					<el-option
						v-for="(item, k) in DictRef.AppGoodsInfoTypesDict"
						:key="k"
						:value="item.value"
						:label="item.label"
					/>
...
export default defineComponent({
	setup() {
		...
		const upsert = reactive<Upsert>({
			items: [
				...
				},
				{
					...
					component: {
						name: "el-radio-group",
						options: Dict.AppGoodsInfoTypesDict
					}
				},
				{
					prop: "status",
		...
		const table = reactive<Table>({
			...
			columns: [
				...
				},
				{
					prop: "types",
					label: "类型",
					dict: Dict.AppGoodsInfoTypesDict,
					align: "center"
				},
				{
					prop: "status",
					label: "状态",
					dict: Dict.AppGoodsInfoStatusDict,
					align: "center"
				},
				{
					label: "操作",
		...
		function onLoad({ ctx, app }: CrudLoad) {
			// 绑定 service
			ctx.service(service.appGoodsInfo).done();
			app.refresh();
		}
		/**
		 * 编辑-打开事件
